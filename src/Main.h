...
uint8_t sleepTime[5] = {0, 0, 8, 0, 0};                  // 这里表示设备休眠时间,默认是凌晨0点到早上八点,所以表示为0:0 ~ 8:0,最后一位是亮度(0:表示息屏,15表示最亮了)
bool isSleepMode = false;                                // 标记当前是否处于睡眠模式
uint32 lastmill = 0, resetmill = 0;
...
void showHumidity()
{
    ...
    displayData[1] = t / 100;
    displayData[0] = t % 100;
    lattice.showHumidity(displayData);
}
